...
The \enquote{fs} library grants access to the filesystem \cite{nodejsFS}. \revMark{It is used to check that} the Windows start menu folder for this application exists and if not, create it (\mylineref{lst:setupWindowsNotifications.js}{swn_fs}). Without the \gls{asp} server this library would  be used to access the music files.

The \enquote{os} library provides access to operating system specific information and methods \cite{nodejsOS}. It is used to determine the current operating system and architecture to be able to register the correct notification libraries.
